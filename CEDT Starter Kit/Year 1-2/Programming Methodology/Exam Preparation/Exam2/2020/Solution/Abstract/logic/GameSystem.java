...
public class GameSystem {
	...
	public void action(Piece piece, boolean attack, int actionPattern) {
		//คือยานแม่ มันจะเคลื่อนไปที่ตำแหน่งที่จะโจมตี แต่่Fighterมันจะตีตำแหน่งที่ห่างจากตัวเองเป็นคู่อันดับ
	  if(piece instanceof MotherShip){
		  if(attack) {
			  Position targetPosition=piece.attackTargetPosition(actionPattern);
			  removeOtherPieces(piece,targetPosition);//เอาตัวอื่นออก
		  }
		  else{
		  ...
		  boolean removed=removeOtherPieces(piece,piece.getCurrentPosition()); //ถ้าตีไปจุดที่มีตัวอื่นอยู่จะpromote fighter
		  if(removed){
			  ((Fighter) piece).setPromoted(true);//อันนี้คือการบอกเรียกmethod ของตัวsubclassที่ไม่มีในตัวSuperclass
		  }
	  }
